...
/* harmony import */ var _ChatPreview__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./ChatPreview */ "./src/forum/components/ChatPreview.js");
...
/*#__PURE__*/
function (_Component) {
  Object(_babel_runtime_helpers_esm_inheritsLoose__WEBPACK_IMPORTED_MODULE_1__["default"])(ChatFrame, _Component);
  ...
  _proto.init = function init() {
    ...
    if (!app.forum.attribute('pushedx-chat.settings.censor') || app.session.user) {
      if (app.pusher) {
        app.pusher.then(function (channel) {
          _this.pusherChannel = channel;

          _this.pusherAttach(channel);
        });
  ...
  _proto.pusherAttach = function pusherAttach(channels) {
    var _this2 = this;
    ...
    ;
    this.pusherChannels = [channel('pushedx-chat.socket.event.post', function (data) {
    ...
    }), channel('pushedx-chat.socket.event.edit', function (data) {
    ...
    }), channel('pushedx-chat.socket.event.delete', function (data) {
      if (!app.session.user || data.actorId != app.session.user.id()) _this2.messageDelete(data);
    })];
    this.pusherChannels.map(function (channel) {
      return channels.main.bind(channel.name, channel.callback);
    });
  };

  _proto.pusherDetach = function pusherDetach(channels) {
    this.pusherChannels.map(function (channel) {
      return channels.main.unbind(channel.name);
    });
  };
  ...
  _proto.view = function view() {
    ...
    })))), m(_ChatPreview__WEBPACK_IMPORTED_MODULE_5__["default"], {
      active: true
    }), m(_ChatPreview__WEBPACK_IMPORTED_MODULE_5__["default"], null), m(_ChatPreview__WEBPACK_IMPORTED_MODULE_5__["default"], null)), m("div", {
      id: "chat-panel"
    }, m("div", {
      id: "chat-header",
    ...
    })) : null, this.messages.components.concat(this.input.writing ? this.input.preview.component : null)), m("div", {
      className: "input-wrapper"
    }, m("textarea", {
      type: "text",
      id: "chat-input",
  ...
  _proto.inputProcess = function inputProcess(e) {
    ...
    this.timedRedraw(100, function () {
      return _this3.scroll.autoScroll && !_this3.messageEditing ? _this3.scrollToBottom() : null;
    });
  };
    ...
    if (!this.input.preview.component) {
      this.input.preview.component = this.createMessage({
        ...
        instanceGetter: function instanceGetter(instance) {
          return _this4.input.preview.instance = instance;
        }
      });
    }
  ...
  _proto.messageRestore = function messageRestore(data) {
    ...
    if (message) {
      ...
      messageIds.some(function (value, index, array) {
        if (array[index - 1] < data.id && data.id < array[index]) return _this5.messages.components.splice(index, 0, _this5.createMessage(data));
      });
      m.redraw();
    }
  };
  ...
  _proto.messageResend = function messageResend(instance) {
    ...
    this.apiPost(instance.message, this).then(function (result) {
      if (result) {
        _this6.messages.components.some(function (value, index, array) {
          if (value == instance.component) return array.splice(index, 1) && array.push(instance.component) && _this6.scrollToBottom();
        });
  ...
  _proto.apiPost = function apiPost(text, targetInstance) {
    ...
    return app.request({
      ...
      _this8.messages.instances[targetInstance.id] = targetInstance;
      if (targetInstance == _this8.input.preview.instance) _this8.input.preview.instance = null;
  ...
  _proto.apiFetch = function apiFetch(lastMessageId) {
...
function (_Component) {
  ...
  _proto.view = function view() {
    return m("div", {
      ...
      className: "name",
      onclick: this.chatFrame.insertMention.bind(this.chatFrame, this)
  ...
  _proto.editDropDown = function editDropDown() {
    return m("div", {
      ...
      icon: "fas fa-reply"
    }, app.translator.trans('core.forum.post_controls.restore_button')), m(flarum_components_Separator__WEBPACK_IMPORTED_MODULE_11___default.a, null)] : m("div", null), !this.deleted_by && this.chatFrame.permissions["delete"] ? m(flarum_components_Button__WEBPACK_IMPORTED_MODULE_10___default.a, {
      onclick: this["delete"].bind(this),
      icon: "fas fa-trash-alt",
      disabled: !this.chatFrame.permissions["delete"]
    }, app.translator.trans('core.forum.post_controls.delete_button')) : m("div", null), this.deleted_by && this.chatFrame.permissions.moderate["delete"] ? m(flarum_components_Button__WEBPACK_IMPORTED_MODULE_10___default.a, {
...
function (_Component) {
  ...
  _proto.view = function view() {
    return m("div", {
      className: 'panel-preview ' + (this.active ? 'active' : '')
    });
  };
