...
function (_Component) {
  ...
  _proto.view = function view() {
    ...
    })) : null, this.getStoragedMessages().concat(this.input.writingPreview ? this.input.preview.component : null)), m("div", {
      className: "input-wrapper"
    }, m("textarea", {
      type: "text",
      id: "chat-input",
  ...
  _proto.messageRestore = function messageRestore(message) {
    ...
    var instance = this.messages.instances[message.id()];
    if (instance) message.restore();else {
      Object.keys(this.messages.instances).some(function (value, index, array) {
        if (array[index - 1] < data.id && data.id < array[index]) return _this3.chatFrame.messagesStorage.splice(index, 0, _this3.createMessage(message));
      });
      m.redraw();
    }
  };
  ...
  _proto.messageNotify = function messageNotify(message) {
    if (!app.session.user || message.user.id() != app.session.user.id()) this.notifyTry(message);
    message.flash();
  };
  ...
  _proto.messageIsMention = function messageIsMention(message) {
    return app.session.user && message.message().indexOf('@' + app.session.user.username()) >= 0;
  };

  _proto.notifyTry = function notifyTry(message) {
    if (!("Notification" in window)) return;
    if (this.messageIsMention(msg)) this.notifySend(message);
    this.notifySound(message);
  };

  _proto.notifySend = function notifySend(message) {
    if (this.notify) !this.active ? new Notification(message.user().username(), {
  ...
  _proto.notifySound = function notifySound(message) {
    if (!this.isMuted) {
      var sound = this.messageIsMention(message) ? refAudio : audio;
      sound.currentTime = 0;
      sound.play();
    }
  };
